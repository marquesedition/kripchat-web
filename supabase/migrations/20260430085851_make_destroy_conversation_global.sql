...
begin
  ...
  values (
    current_user_id,
    target_id,
    'destroy_conversation_requested',
    jsonb_build_object(
      'scope', 'global_conversation',
      'effect', 'remove_for_all_members_and_devices',
      'storage_cleanup', 'skipped_in_sql',
      'reason', 'Supabase Storage objects must be deleted with the Storage API'
    )
  );

  -- Explicit global deletion. These deletes are intentionally by conversation_id,
  -- not by actor id, so the channel disappears for every member/device.
  ...
  values (
    current_user_id,
    null,
    'destroy_conversation_completed',
    jsonb_build_object(
      'conversation_id', target_id,
      'scope', 'global_conversation',
      'effect', 'removed_for_all_members_and_devices',
      'storage_cleanup', 'skipped_in_sql'
    )
  );
